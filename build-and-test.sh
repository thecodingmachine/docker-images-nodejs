...
docker buildx build --platform=linux/amd64,linux/arm64 --load -t thecodingmachine/nodejs:${BRANCH}-${VARIANT} -f Dockerfile.${VARIANT} .
...
docker buildx build --platform=linux/amd64,linux/arm64 --load -t thecodingmachine/nodejs:${VARIANT} -f Dockerfile.${VARIANT} .
